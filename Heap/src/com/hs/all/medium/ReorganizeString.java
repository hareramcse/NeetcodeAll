...
public class ReorganizeString {
	public String reorganizeString(String s) {
		...
		while (!pq.isEmpty()) {
			...
			// if the character at sb's end is different from the max frequency character or
			// the string is empty
			if (sb.length() == 0 || sb.charAt(sb.length() - 1) != temp1.getKey()) {
				...
				// if there is no temp2 i.e. the temp1 was the only character in the heap then
				// there is no way to avoid adjacent duplicate values
				if (temp2 == null)
					return "";
